...
Future<PackageSummary> summarizePackageArchive(
  ...
  /// Knobs toggling which pub.dev-derived validators run. Defaults to the
  /// strict upstream behaviour; pass [ReaderPolicy.club] to match club's
  /// private-registry tuning (LICENSE/README optional, templates allowed,
  /// `publish_to` not enforced).
  ReaderPolicy policy = ReaderPolicy.pubDev,
}) async {
  ...
  final rawPubspecContent = await tar.readContentAsString(pubspecPath);
  final pubspecContent = policy.applyPubSemverOverride
      ? overridePubspecYamlIfNeeded(
  ...
  if (policy.checkPublishTo) {
    issues.addAll(validatePublishTo(pubspec.publishTo));
  }
  issues.addAll(validateDescription(pubspec.description, policy: policy));
  ...
  if (policy.forbidGitDependencies ||
      policy.forbidNonDefaultHostedDependencies) {
    issues.addAll(
  ...
  if (policy.rejectKnownTemplateReadmes) {
...
///
/// Upstream checks `knownMixedCasePackages` / `blockedLowerCasePackages` to
/// grandfather legacy pub.dev packages; those tables were dropped when
/// vendoring since a private registry has no such history. The remaining
/// rules — reserved-word rejection and all-lowercase — apply universally.
...
Iterable<ArchiveIssue> validateDescription(
  String? description, {
  ReaderPolicy policy = ReaderPolicy.pubDev,
}) sync* {
  if (description == null) {
  ...
  if (policy.rejectEmojiInDescription && hasEmojiCharacter(description)) {
    yield ArchiveIssue(
      '`description` is not allowed to have emoji characters.',
    );
  }
  if (policy.rejectKnownTemplateDescriptions) {
...
/// Validate that the package does not have any git dependencies, and that
/// hosted dependencies target a trusted registry.
///
/// It ignores `dev_dependencies` as these are for development only.
///
...
///   `hosted: url:`. Only used for the error message; [isHostedUrlAllowed]
///   does the actual matching (normalises origins before comparing).
Iterable<ArchiveIssue> forbidGitDependencies(
  ...
  for (final entry in pubspec.dependencies.entries) {
    ...
    if (!forbidNonDefaultHosted) {
      continue;
    }

    if (entry.value is! HostedDependency) {
      yield ArchiveIssue(
        'Package dependency $name is not hosted on a trusted pub server',
      );
      continue;
    }
    ...
    if (dep.hosted == null) {
      // No explicit `hosted:` — defaults to pub.dev, always allowed.
      continue;
    }
    final explicitUrl = dep.hosted!.url?.toString();
    if (explicitUrl != null) {
      final allowed = isHostedUrlAllowed != null
          ? isHostedUrlAllowed(explicitUrl)
          : false;
      if (!allowed) {
        yield ArchiveIssue(
          'Package dependency $name is hosted on $explicitUrl, which is not '
          'in the allowed-hosts list (${allowedHostedUrls.join(', ')}).',
        );
      }
    }
    final explicitName = dep.hosted!.name;
    if (explicitName != name) {
      yield ArchiveIssue(
        'Package dependency $name declares `hosted.name: $explicitName`, '
        'which does not match the dependency key.',
      );
    }
  }
}
