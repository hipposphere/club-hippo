...
void main() {
  group('package name validation', () {
    ...
    // Upstream pub.dev grandfathers legacy mixed-case names via
    // `knownMixedCasePackages` / `blockedLowerCasePackages`. Those tables were
    // stripped when vendoring (see src/names.dart header), so the rule here
    // is simply "lowercase required", with no per-package exceptions.
    test('reject any mixed-case name', () {
      expect(validateNewPackageName('Babylon'), isNotEmpty);
      expect(validateNewPackageName('Pong'), isNotEmpty);
      expect(validateNewPackageName('myNewPackage'), isNotEmpty);
    });

    test('accept lowercase pong and babylon unconditionally', () {
